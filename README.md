...
Version 1.2 uses Apple's Liquid Glass APIs for the functional and navigation layer on macOS 26 or newer. Buttons, segmented navigation, pickers, the audio scrubber, chat composer, recording controls, and speaker tools use native or grouped interactive glass. macOS 14–15 receive an equivalent system-material presentation with the same hierarchy, accessibility, and behavior.

The main window follows a focused two-column Mac pattern: destinations and searchable recordings stay in the resizable sidebar while the selected recording gets the full workspace. Selection and the active Notes, Transcript, or Ask view persist naturally between navigation changes. Empty, processing, stale, and failed states provide their next relevant action in context.
...
1. Open Settings.
2. Add an OpenAI API key and select **Validate**. The key is saved in macOS Keychain.
3. Record a conversation or import an audio file, then confirm you are permitted to record/process it.
4. Select a template and choose **Process**.
...
- No realtime transcription, hidden/background auto-recording, cloud sync, accounts, mind maps, or global semantic search are included.
